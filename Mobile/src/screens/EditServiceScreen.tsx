...
type RouteT = RouteProp<RootStackParamList, 'EditService'>;
...
export default function EditServiceScreen() {
  ...
  const mutation = useMutation({
    mutationFn: (data: Form) => serviceService.update(id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: QUERY_KEYS.SERVICES });
      queryClient.invalidateQueries({ queryKey: QUERY_KEYS.SERVICE(id) });
      Alert.alert('Sucesso', 'Serviço actualizado!');
      navigation.goBack();
    },
    onError: (err: any) => {
      Alert.alert('Erro', err.response?.data?.message || 'Erro ao actualizar serviço');
    },
  });
