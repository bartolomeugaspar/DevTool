...
export default function EditService() {
  ...
  const {
    ...
    values: service
      ? { nome: service.nome, descricao: service.descricao, preco: service.preco }
      : undefined,
  });

  const mutation = useMutation({
    mutationFn: (data: EditServiceForm) => serviceService.update(id!, data),
    onSuccess: () => {
    ...
    onError: (err: unknown) => {
      ...
      setServerError(error.response?.data?.message || error.response?.data?.error || 'Erro ao editar serviço');
    },
  });
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="rounded-2xl p-5 sm:p-8 space-y-5"
        ...
        <div className="flex gap-3 pt-2">
          ...
          <button
            ...
            className="flex-1 text-white font-medium py-2.5 rounded-xl transition-all text-sm disabled:opacity-60"
